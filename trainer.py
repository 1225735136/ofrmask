...
class Trainer:
    def __init__(self, options):
        ...
        ####--------------------Using Flow Net (Manydepth2)--------------------')
        self.models["optic_flow_mask"] = networks.OpticFlowMask()
        self.models["optic_flow_mask"].to(self.device)

        ##################
        ###----------------------teacher Network-------
        # with open("/home/jsw/Lite-Mono-optic_flow/networks/idisc/models/kitti_swinl.json", "r") as f:
        #     config = json.load(f)
        # self.models["teacher"] = networks.IDisc.build(config)
        # self.models["teacher"].to(self.device)
        # state_dict = torch.load('/home/jsw/Lite-Mono-optic_flow/networks/idisc/KITTIDataset-best.pt',
        #                         map_location='cpu')
        # self.models["teacher"].load_state_dict(state_dict)
        # for param in self.models["teacher"].parameters():
    ...
    def process_batch(self, inputs,epoch):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            # pseudo_depth, _, _ = self.models["teacher"](image=inputs["color_aug", 0, 0],
            #                                             gt=None, mask=None)
        if self.opt.predictive_mask:
        ...
        outputs.update(self.models["optic_flow_mask"](inputs, outputs, epoch))
    ...
    def val(self):
        ...
        with torch.no_grad():
            outputs, losses = self.process_batch(inputs,self.epoch)
    ...
    def compute_losses(self, inputs, outputs, epoch):
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                reprojection_losse=self.compute_reprojection_loss(pred, target)
                # if epoch > 5:
                reprojection_losse = reprojection_losse*outputs['motion_mask', frame_id, 0]
                reprojection_losses.append(reprojection_losse)
